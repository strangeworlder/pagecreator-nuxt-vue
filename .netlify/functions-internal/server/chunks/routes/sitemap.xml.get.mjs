import { d as defineEventHandler, f as useRuntimeConfig, q as queryCollection, s as setHeader } from '../_/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
import 'node:buffer';
...
const sitemap_xml_get = defineEventHandler(async (event) => {
  ...
  for (const d of docs) {
    ...
    let lastmodDate = d.dateModified ? new Date(d.dateModified) : /* @__PURE__ */ new Date();
